...
- If the current checkout is already the dedicated worktree for that feature,
  continue there instead of creating another one.
...
- In a non-interactive environment where Worktrunk cannot change the parent
  shell's directory, add `--no-cd --format=json`, read the `path` field from
  the result, and run all subsequent commands with that directory as their
  working directory.
